...
// Buscar video en YouTube usando play-dl
async function searchYouTube(query) {
  try {
    const results = await play.search(query, { limit: 5 });
    if (!results || results.length === 0) return null;
    const video = results[0];
    return { url: video.url, title: video.title, duration: video.durationInSec };
  } catch (err) {
  ...
  if (commandName === 'play') {
    ...
    try {
      ...
      if (channelData.player.state.status === AudioPlayerStatus.Idle && channelData.queue.length === 0) {
        await playSong(channelData, video);
      ...
      await interaction.editReply('⚠️ No se pudo reproducir ese video. Intenta con otro.');
    }
  }
